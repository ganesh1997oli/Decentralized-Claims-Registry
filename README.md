...
On-chain insurance-claims registry (Solidity / Hardhat 3) with an off-chain Python listener that feeds claim events to an AI fraud-detection pipeline and writes verdicts back on-chain.
...
All tests must pass before anything else. (npx hardhat compile also fixes editor errors like "'claim' is of type 'unknown' after a fresh clone.")
...
Terminal C - submit a claim and write fraud verdict back
